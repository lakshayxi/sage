...
- Eval harness: `.venv/bin/sage-eval [--limit N]` — scores the RAG pipeline against `eval/dataset.py`'s hand-curated ~18-item Q&A set (check the file for the exact count, it will drift). Every run passes `use_cache=False` to `generate_answer` so cached answers can't mask a same-day prompt/parsing fix as passing. Requires a live `GEMINI_API_KEY` and costs real Gemini calls; not part of `pytest tests/`.
...
- Retrieval is hybrid (BM25 + vector via RRF, `sage/retrieval/retriever.py:retrieve_hybrid`) then cross-encoder reranked (`sage/retrieval/reranker.py`, `BAAI/bge-reranker-base`) and gated by `MIN_RERANK_SCORE` (0.1) — if every candidate scores below it, the pipeline refuses without ever calling the LLM. This is a hard cutoff, not model-judged, and can false-reject vague/conversational phrasing even when the data exists; a narrow one-time retry strips a trailing evaluative clause ("...were they good?") and retries once, but that doesn't cover every phrasing.
